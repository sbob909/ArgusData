...
ArgusData.prototype.queryArgus = function(e,type,tagForSeries){
  var deferred = Q.defer();
  var accessToken = localStorage.getItem('ngStorage-accessToken').slice(1,accessToken.length-1);
  $.ajax({
    url: e.url,
    async: true,
    crossDomain: true,
    ...
    success: function (data) {
      ...
      if (type == 'tag'){
        ...
        e.jsonData.sort(function(a, b){
          return a['tags'][tagForSeries] > b['tags'][tagForSeries];
        });
        ...
        e.jsonData.forEach(function(currentValue){
          ...
          e.seriesNames.push(currentValue['tags'][tagForSeries]);
        });
      }
      deferred.resolve();
    }
